...
public class AppointmentDao {
	...
	public boolean addAppointment(Appointment ap) {
		...
		try {
			...
			String sql= "insert into appointment(user_id,fullname,gender,age,appoint_date,email,phone,diseases,doctor_id,address,status) values(?,?,?,?,?,?,?,?,?,?,?)";
	...
	public Appointment getAppointmentById(int appoint_id){
		Appointment ap = null;
		try {
	...
	public boolean updateCommentStatus(int id, int docId, String comm) {
		...
		try {
			...
			String sql="update appointment set status=? where appoint_id=? and doctor_id=?";
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setString(1, comm);
			ps.setInt(2, id);
			ps.setInt(3, docId);
	...
	public List<Appointment> getAllAppointment(){
		...
		try {
			...
			String sql = "select * from appointment order by appoint_id desc";
			PreparedStatement ps = conn.prepareStatement(sql);
